...
describe('Layout', () => {
  ...
  it('render the pokemon image', () => {
    expect(pokemonCard.find('img').attributes('src')).toContain('https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/1.png');
  });
  ...
  it('use the color of the first pokemon type', () => {
    expect(getComputedStyle(pokemonCard.element).getPropertyValue('--pokemon-color')).toBe('#74CB48');
  });
